...
export default function App() {
  ...
  function deductForIngredients(names) {
    setItems(prev => prev.map(x => {
      ...
      if (names.some(n=>k.includes(n))) {
        const dec = x.sizeValue ? x.sizeValue*0.25 : 1; // rough default 25% or 1 unit
        return { ...x, remaining: Math.max(0, (x.remaining||x.qty) - dec) };
      }
      return x;
    }));
    closeUseModal();
  ...
  const [pendingSizeFor, setPendingSizeFor] = useState(null); // item id awaiting size
  ...
  async function addByBarcode(barcode) {
    ...
    const newItem = { id:`${barcode}-${now.getTime()}`, gtin:base.gtin, name:base.name, brand:base.brand, notedQuantity:base.quantity,
      sizeValue: 0, sizeUnit: 'ct', remaining: 1,
      qty:1, unit:'unit', category:cat, addedAt:now.toISOString(), shelfDays, expiry:expiry.toISOString() };
    setItems((prev) => [newItem, ...prev]);
    setLoading(false);
    setNotif(`Added ${base.name}`);
  ...
  function addManual(e) {
    ...
    const now = new Date();
    const expiry = addDays(now, shelfDays);
    const newItem = { id:`manual-${now.getTime()}`, gtin:String(fd.get("gtin") || ""), name, brand:String(fd.get("brand") || ""), notedQuantity:String(fd.get("notedQuantity") || ""), qty, unit:String(fd.get("unit") || "unit"), category, addedAt:now.toISOString(), shelfDays, expiry:expiry.toISOString() };
    setItems((prev) => [newItem, ...prev]);
  ...
  // Budget-aware: mock price mapping (can be replaced w/ real store data)
  const PRICE = (name) => {
    const n = name.toLowerCase();
    if (n.includes("chicken")) return 8;
    if (n.includes("beef")) return 10;
    if (n.includes("salmon") or n.includes("fish")) return 12;
    ...
    if (n.includes("onion") or n.includes("garlic")) return 2;
    return 5;
  };

  // Build shopping list with budget consideration
  function buildShoppingList() {
    ...
    for (const u of singleUnlocks.slice(0,5)) {
      list.set(u.ingredient.toLowerCase(), Math.max(list.get(u.ingredient.toLowerCase()) || 0, 1));
    }
    for (const p of pairUnlocks.slice(0,5)) {
      for (const ing of p.pair) list.set(ing.toLowerCase(), Math.max(list.get(ing.toLowerCase()) || 0, 1));
    }
    const entries = Array.from(list.entries()).map(([name, qty]) => ({ name, qty, price: PRICE(name), reasons: [] }));
    const byName = new Map(entries.map(e=>[e.name,e]));
    for (const i of lowOrExpiring) { const key = i.name.toLowerCase(); if (byName.has(key)) byName.get(key).reasons.push(`refill (qty ${i.qty}, exp ${fmt(i.expiry)})`); }
